...
class TodayScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TodayBloc, TodayState>(builder: (context, state) {
      switch (state.runtimeType) {
        case TodayLoadingState:
          return const Center(
            child: CircularProgressIndicator(),
          );
        case TodayDataLoaded:
          final todos = (state as TodayDataLoaded).todos;
          return BlocListener<ToDoBloc, ToDoState>(
            listener: (_, addToDoState) {
              if (addToDoState is NewToDoAddedState) {
                context.read<TodayBloc>().add(TodayUpdateDataEvent());
              }
            },
            child: Scaffold(
              appBar: AppBar(
